...
`chain_flow.vllm_plugin.async_guard` rebinds a PRIVATE vLLM symbol, so its blast radius and
...
from chain_flow.vllm_plugin import async_guard  # noqa: E402
...
def test_opting_out_does_not_touch_vllm(guard, monkeypatch):
    ...
    monkeypatch.setattr("chain_flow.defaults._APPLIED", True)   # don't re-default it to 1
    guard.register()
...
def _fake_config(monkeypatch, requested, model="chain_flow.vllm_plugin.flow_proposer.X"):
...
def test_somebody_elses_custom_class_proposer_is_left_alone(guard, monkeypatch):
    ...
    fatal check just because chain-flow happens to be installed."""
