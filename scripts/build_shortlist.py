...
inside the wheel as `src/chained_flow/data/shortlist_qwen3_5.pt`).

The generic builder now lives in the PACKAGE (`chained_flow.shortlist.build`, exposed as
`chained-flow build-shortlist`), because the fallback warning a pip user sees has to name a
...
Sources (union -- see chained_flow/shortlist.py for why it is a union and not a top-K):
...
from chained_flow.shortlist import build  # noqa: E402
