"""Eval for the Markov-flow drafter (V6): cached per-position accept@i (teacher-forced AR,
...
from chained_flow.training.train_markov_flow import load_markov_module
...
def main() -> None:
    ...
    module, config = load_markov_module(args.flow_dir, frozen_lm=frozen_lm, device=device)
    ...
    for i, batch in enumerate(loader):
        ...
        for i in range(k):
            if prev_tok is not None:
                drafter_logits[:, i, :] = base_logits[:, i, :] + drafter.markov.bias(prev_tok)
            prev_tok = drafter_logits[:, i, :].argmax(dim=-1)
