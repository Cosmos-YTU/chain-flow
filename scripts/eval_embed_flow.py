"""Eval for the embedding-space flow matcher (V5): cached per-position accept@i (teacher-forced AR,
...
from chained_flow.training.train_embed_flow import load_embed_module
...
def main() -> None:
    ...
    module, config = load_embed_module(args.flow_dir, frozen_lm=frozen_lm, device=device)
    ...
    for i, batch in enumerate(loader):
        ...
        # cached eval uses the INFERENCE path: init z0 + full flow integration (no target leak),
        # exactly what propose() does. Target embeddings only used as the metric reference.
        b, k = future_tokens.shape
        context_kv = drafter._context_kv(context_hidden)
        z0 = drafter.init_z0(context_kv, b)
        pred_embed = drafter.integrate(context_kv, z0)
        target_embed = drafter._embed_tokens(future_tokens)
        drafter_logits = module.lm_head(pred_embed)
        teacher_logits = module.lm_head(target_embed)
        metrics = per_token_flow_metrics(
            pred_hidden=pred_embed, target_hidden=target_embed,
            pred_latent=pred_embed, target_latent=target_embed,  # no VAE: latent == embed
