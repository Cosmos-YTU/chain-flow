"""Push the expanded-data Qwen3.5-4B drafter to selimaktas/Flow-Drafter-Qwen3.5-27B-v2, with a card carrying
...
CKD = ROOT / "out/flow/ckpts/tree-vae-joint-q3527bx-1024-k8-l8"
VAE = ROOT / "out/vae/ckpts/transformer-hidden-q3527bx-5120-latent1024-fp16"
REPO = "selimaktas/Flow-Drafter-Qwen3.5-27B-v2"
STAGE = Path("/tmp/flow-drafter-q3527b-v2-push")
...
def table():
    new = acc(ROOT / "logs/deploy_eval_q3527bx.log")
    old = acc(ROOT / "logs/deploy_eval_q3527b.log")
...
Expanded-data revision of [Flow-Drafter-Qwen3.5-27B](https://huggingface.co/selimaktas/Flow-Drafter-Qwen3.5-27B) — a
speculative-decoding draft model for `Qwen/Qwen3.5-27B` (Chained-Flow, joint-VAE tree drafter).

**What changed in v2:** trained on a broader, diversity-weighted mix (~30k rows, 10 sources) that adds
...
def main():
    ...
    api.upload_folder(folder_path=str(STAGE), repo_id=REPO, repo_type="model",
                      commit_message="Flow-Drafter-Qwen3.5-27B-v2 — expanded diverse-data drafter")
