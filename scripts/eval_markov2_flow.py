"""Eval for the Markov2-flow drafter (V7): cached per-position accept@i (teacher-forced AR,
...
from chained_flow.training.train_markov2_flow import load_markov2_module
...
def main() -> None:
    ...
    module, config = load_markov2_module(args.flow_dir, frozen_lm=frozen_lm, device=device)
    ...
    for i, batch in enumerate(loader):
        ...
        # cached eval = INFERENCE path: parallel flow -> base logits -> autoregressive markov
        # correction using the drafter's OWN decoded tokens (NO target leak), matching propose().
        b, k = future_tokens.shape
        pred_hidden = drafter.predict_hidden(context_hidden)
        base_logits = module.lm_head(pred_hidden)                 # [B,K,V]
        drafter_logits = base_logits.clone()
        hist = []  # decoded tokens, most-recent appended last
        order = drafter.config.markov_order
        for i in range(k):
            if hist:
                prev = [hist[-1 - j] for j in range(order) if len(hist) > j]
                drafter_logits[:, i, :] = base_logits[:, i, :] + drafter.markov.bias(prev, pred_hidden[:, i, :])
            hist.append(drafter_logits[:, i, :].argmax(dim=-1))
