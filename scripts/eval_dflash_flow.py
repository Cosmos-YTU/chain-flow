"""Eval for the DFlash-style drafter (V4): cached per-position accept@i (teacher-forced AR,
...
from chained_flow.training.train_dflash_flow import load_dflash_module
...
def main() -> None:
    ...
    module, config = load_dflash_module(args.flow_dir, frozen_lm=frozen_lm, device=device)
    ...
    for i, batch in enumerate(loader):
        ...
        # cached eval uses the INFERENCE path (fully-masked block), not random-mask training,
        # so the metric matches what propose() does at generation time (no target leak).
        b, k = future_tokens.shape
        full_mask = torch.ones(b, k, dtype=torch.bool, device=device)
        block_emb = drafter._build_block_emb(torch.zeros(b, k, dtype=torch.long, device=device), full_mask)
        pred_hidden = drafter.denoise(block_emb, context_hidden)
