    ...
    if built is not None:
        if int(built) != int(vocab_size):
            ...
            return (f"built for vocab_size={int(built)} but this model's lm_head has "
                    ...
                    f"`chain-flow build-shortlist --vocab-size {int(vocab_size)} ...`: the "
...
def build(argv=None) -> int:
    """``chain-flow build-shortlist`` -- the command the fallback warning names.
    ...
    ap = argparse.ArgumentParser(
        prog="chain-flow build-shortlist",
