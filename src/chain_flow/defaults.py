...
* ``CF_ASYNC_SPEC`` is read while ``LLM(...)`` is still being constructed, inside
  ...
  (``chain_flow.vllm_plugin.async_guard``), which runs in ``EngineArgs.__post_init__`` and
...
``apply()`` is therefore called from ``chain_flow/__init__.py`` and is idempotent.
...
DRAFTER: dict[str, str] = {
    ...
    # default may actually be used lives in `chain_flow.shortlist.check`, because it needs the
    ...
    # `chain_flow.vllm_plugin.async_guard` -- a `vllm.general_plugins` entry point -- relaxes
...
POLICY: dict[str, str] = {
    "CF_TREE_GREEDY_GUARD":
        ...
        "See chain_flow/vllm_plugin/greedy_guard.py.",
}
...
def repo_root() -> Path:
    # src/chain_flow/defaults.py -> src/chain_flow -> src -> <repo>
    return Path(__file__).resolve().parents[2]


def packaged_shortlist() -> Path:
    """The 250 KB int32 list that ships INSIDE the wheel.  Duplicated from
    ``chain_flow.shortlist.packaged_path`` on purpose: this module must stay importable as a
    bare FILE with no ``chain_flow`` package and no torch (see ``_sh``)."""
...
def _shortlist_default() -> str:
    ...
    checkpoint dir and the real vocab size; this value is what reaches ``chain-flow env`` and
...
def apply(force: bool = False) -> None:
    ...
    if sl is None:
        ...
        if sl:
            ...
            # including a plain `vllm serve` that has chain-flow installed and is not using
            ...
            if os.environ.get("CF_DRAFTER_DIR"):
                print("[cf-defaults] WARNING: CF_SHORTLIST is unset and no shortlist was found "
                      ...
                      "build one with `chain-flow build-shortlist`.", flush=True)
    else:
...
def finalize_async(p, vllm_config) -> bool:
    ...
    * on STOCK vLLM, ``chain_flow.vllm_plugin.async_guard`` (a ``vllm.general_plugins`` entry
    ...
    * on the FORK, ``config/vllm.py`` has the same conditional inline, but only on the
    ...
    if not getattr(vllm_config.scheduler_config, "async_scheduling", False):
        disable("CF_ASYNC_SPEC",
                ...
                "(pip install chain-flow, and check for the '[cf-plugin] ... guard relaxed' "
...
def _sh() -> str:
    ...
    Runs as a FILE, not ``-m``: executing the file directly skips ``chain_flow/__init__``
    ...
    invocation and false for ``chain-flow env``, because importing the package runs ``apply()``
    (see ``chain_flow/__init__``) and therefore puts the whole table into ``os.environ`` before
    this function is ever called.  Reading the live environment there made the emitter skip every
    flag and print an empty export list: a command documented as ``eval "$(chain-flow env)"``
    ...
    for flag in _KEYS:
        ...
        if flag == "CF_SHORTLIST":
            ...
            # to prevent.
            #
            # Nothing is lost by omitting it: the child imports `chain_flow`, runs `apply()`,
