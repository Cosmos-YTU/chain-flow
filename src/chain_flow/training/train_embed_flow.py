...
def train_embed_with_trainer(model_args, data_args, loss_args, training_args) -> dict[str, Any]:
    ...
    with (output_dir / "chain_flow_embed_config.json").open("w", encoding="utf-8") as f:
...
def load_embed_module(flow_dir: str | Path, *, frozen_lm: FrozenLMWrapper, device):
    ...
    config_path = flow_dir / "chain_flow_embed_config.json"
    if not config_path.exists():
        config_path = flow_dir.parent / "chain_flow_embed_config.json"
