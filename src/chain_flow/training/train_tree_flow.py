...
from chain_flow.drafters.tree_flow import TreeFlowConfig, TreeFlowDrafter
...
class TreeFlowTrainingModule(nn.Module):
    def __init__(self, frozen_lm: FrozenLMWrapper, dcfg: TreeFlowConfig, loss_cfg: FlowLossArguments):
        ...
        if _truthy(os.environ.get("CF_COMPILE", "1")):
            ...
            if mode == "max-autotune" and _truthy(os.environ.get("CF_FUSED_HEAD", "1")):
                ...
                print("[chain-flow] CF_COMPILE_MODE=max-autotune is unsafe with the fused head "
            ...
            try:
                ...
                print(f"[chain-flow] training: torch.compile(drafter.forward_teacher, "
                      f"mode={mode})", flush=True)
            except Exception as e:                                        # noqa: BLE001
                print(f"[chain-flow] torch.compile unavailable ({e!r}); running eager", flush=True)
        print(f"[chain-flow] training: fused_head={self._fused_head} chunk={self._fused_chunk}",
              flush=True)
    ...
    def forward(self, context_hidden, target_hidden, future_tokens,
        ...
        if use_fused:
            from chain_flow.training.fused_head import (accept_from, ce_from, coverage_from,
                                                          head_reductions)
