...
from chain_flow.training.train_vae import HiddenVAETrainingModule, VAELossArguments, VAEModelArguments
from chain_flow.training.vae_dataset import HiddenTokenTensorDataset, TeacherHiddenTokenDataset, collate_hidden_tokens
...
def find_vae_config_dir(vae_dir: str | Path) -> Path:
    ...
    for candidate in (vae_dir, vae_dir.parent):
        if (candidate / "chain_flow_vae_config.json").exists():
            return candidate
...
def load_vae_training_module(vae_dir: str | Path, *, device: torch.device) -> HiddenVAETrainingModule:
    ...
    config_path = config_dir / "chain_flow_vae_config.json"
...
def load_flow_cache_hidden_tokens(path: str | Path, *, response_only: bool = True) -> torch.Tensor:
    from chain_flow.training.window_dataset import FLOW_CACHE_FILES, is_flow_window_cache
...
def load_vae_eval_dataloader(args: VAEEvalArguments) -> DataLoader:
    from chain_flow.training.window_dataset import is_flow_window_cache
