...
#: ``chain_flow.defaults.finalize_async`` to tell "stock vLLM, guard relaxed" apart from
...
def _wanted() -> bool:
    ...
    ``chain_flow.defaults`` owns the value; importing it is cheap (stdlib only, no torch) and
...
def register() -> None:
    ...
    that merely has chain-flow on disk and no intention of using it.  Taking that engine down
    ...
    try:
        from chain_flow.vllm_plugin import greedy_guard
    ...
    if not str(version).startswith(_SUPPORTED):
        ...
        print(f"[cf-plugin] NOT relaxing vLLM's async-scheduling guard: {REASON}. A "
              f"chain-flow speculative run on this vLLM will REFUSE to start; anything else "
...
def _install_resolution_check(cfg) -> None:
    ...
    fires only when
      ...
      * the speculative config is OURS (``method="custom_class"`` and a chain-flow proposer
    ...
    def __post_init__(self, *a, **k):
        ...
        if _is_ours(getattr(self, "speculative_config", None)) and requested is None:
            ...
            if resolved is not True:
                raise RuntimeError(
                    "chain-flow needs vLLM's async scheduling for this speculative config "
...
def _is_ours(spec) -> bool:
    """A `custom_class` proposer that is a chain-flow one.
    ...
    for attr in ("model", "draft_model", "custom_class"):
        ...
        if isinstance(v, str) and "chain_flow" in v:
            return True
    return False
