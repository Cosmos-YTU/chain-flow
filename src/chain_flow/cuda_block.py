...
**DEFAULT ON**, capability-gated by ``chain_flow.defaults.finalize_cuda_block``: the shape gate
...
def enabled() -> bool:
    # DEFAULT ON, gated: `chain_flow.defaults` writes this env var DOWN to "0" in
...
def available() -> tuple[bool, str]:
    ...
    try:
        ...
        import sys
        if "chain_flow.vllm_plugin.flow_proposer" not in sys.modules:
