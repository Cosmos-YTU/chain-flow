...
from chain_flow.drafters.markov_flow import MarkovHead
...
class TreeFlowDrafter(nn.Module):
    def __init__(self, frozen_lm: FrozenLMWrapper, config: TreeFlowConfig):
        ...
        self.anchor_in = (nn.Linear(self.hidden_size, config.expert_dim)
                          if getattr(config, "anchor_token", False) else None)   # None => no new keys
    ...
    def integrate(self, context: torch.Tensor, z0: torch.Tensor, anchor=None) -> torch.Tensor:
        ...
        for s in range(self.config.num_flow_steps):
            ...
            z = z + dt * self.flow_velocity(z, tau, context, anchor=anchor)
        return z

    def anchor_embed(self, anchor_token):
        """emb(last committed token) -> the flow's anchor conditioning vector. None when disabled."""
        if anchor_token is None or self.anchor_in is None:
            return None
        return self.anchor_in(self._embed(anchor_token).to(self._dtype))

    def predict_hidden(self, context: torch.Tensor, anchor=None) -> torch.Tensor:
        return self.integrate(context.to(self._dtype), self.init_latents(context), anchor=anchor)
    ...
    def _path_residual(self, tokens: torch.Tensor, prev_token: torch.Tensor | None = None) -> torch.Tensor:
        """Path-conditioned hidden residual for every position given a committed token path [B, K].

        `prev_token` [B] is the LAST COMMITTED token -- position 0's real parent. Without it
        _parent_stack masks position 0 out entirely (it only sees `tokens`), so training supervised
        position 0 with NO path conditioning while inference feeds it the seed token. Position 0 is
        the highest-leverage slot because accept is a prefix, so that mismatch is expensive.
        """
        if prev_token is None:
            ids, mask = self._parent_stack(tokens)
        else:
            ext = torch.cat([prev_token.unsqueeze(1), tokens], dim=1)     # [B, K+1]
            ids, mask = self._parent_stack(ext)
            ids, mask = ids[:, 1:], mask[:, 1:]                           # realign to positions 0..K-1
    ...
    def forward_teacher(self, context: torch.Tensor, target_hidden: torch.Tensor, future_tokens: torch.Tensor,
        ...
        # SCHEDULED SAMPLING: training conditions the path residual / markov bias on the TRUE future
        # tokens, but at inference the beam feeds its OWN predictions back. Mixing the model's own
        # marginal argmax in with probability p closes that exposure gap. p=0 reproduces the old path.
        path_tokens = future_tokens
        p_ss = float(getattr(self.config, "sched_sampling_p", 0.0))
        if self.training and p_ss > 0.0:
            own = base_logits.argmax(dim=-1)                    # [B, K], no extra forward needed
            flip = torch.rand_like(own, dtype=torch.float32) < p_ss
            path_tokens = torch.where(flip, own, future_tokens)

        residual = self._path_residual(path_tokens, prev_token)
        ...
        if prev_token is None:
            bias[:, 0, :] = 0.0                                # legacy: position 0 had no known parent
        else:
    ...
    def build_tree_fast(self, state: LMState, *, top_b: int | None = None, max_nodes: int | None = None,
                        max_depth: int | None = None, forced_root: int | None = None):
        ...
        ``forced_root``: if given, the tree has a SINGLE root node holding that token (instead of
        top_b root candidates). Used by the bonus-decode fold — the previous step's committed bonus
        token becomes this tree's root, so its hidden/state is computed inside THIS verify instead of
        a separate per-step decode. The first token is known-correct, so no root breadth is wasted."""
        cfg = self.config
        top_b = cfg.tree_top_b if top_b is None else top_b
        max_nodes = cfg.tree_max_nodes if max_nodes is None else max_nodes
        ...
        if forced_root is not None:
            idx = torch.tensor([int(forced_root)], device=dev)
            vals = torch.log_softmax(base0.float(), dim=-1)[idx]   # its logprob (cum-ordering seed)
            b0 = 1
        else:
