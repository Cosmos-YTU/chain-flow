"""``chained-flow tree-patch`` -- apply / revert / verify the OPTIONAL forked-vLLM tree path.
...
PIN = "0.25.1"
PATCH_NAME = f"vllm-{PIN}-chained-flow-tree.patch"
...
def locate() -> Target:
    ...
    if spec.origin is None or not (pkg / "__init__.py").is_file():
        ...
        raise PatchError(
            ...
            "      mean to patch:  <venv>/bin/python -m chained_flow.cli tree-patch --status")
...
def load_patch() -> list[FilePatch]:
    ...
    if not p.is_file():
        raise PatchError(
            ...
            f"src/chained_flow/patches/{PATCH_NAME}")
...
def _fork_report() -> str:
    ...
    ``defaults.fork()`` MEMOISES its answer in a module global, and ``chained_flow/__init__``
...
def cmd_apply(t: Target, fps: list[FilePatch], dry: bool) -> int:
    ...
    if not ok:
        print("[cf] POST-APPLY VERIFICATION FAILED -- the install does not look the way this\n"
              "     patch says it should. Run `chained-flow tree-patch --revert`, then\n"
              f"     pip install --force-reinstall --no-deps 'vllm=={PIN}'.", file=sys.stderr)
        for rel, v in verdicts.items():
            if v != PATCHED:
    ...
    print(dedent_block(f"""
        Verify it works:
          chained-flow info                 # expect `vLLM build : FORKED`
        ...
        greedy-only. Undo at any time with:
          chained-flow tree-patch --revert"""))
    return 0
...
def cmd_show() -> int:
    ...
    print("The safe way -- verifies against the wheel's RECORD manifest before and after, and is "
          "reversible:\n")
    print("  chained-flow tree-patch --status")
    print("  chained-flow tree-patch --apply")
    print("  chained-flow tree-patch --revert\n")
    ...
    print("`chained-flow info` reports `vLLM build : FORKED` once it is in.")
    return 0
...
def main(args) -> int:
    ...
    if s is not None:                       # None means the version gate already refused: the
        print("\nApply it with:  chained-flow tree-patch --apply "   # last thing that install
              "   (--dry-run to see it without writing)")            # needs is an invitation
        print("Undo it with :  chained-flow tree-patch --revert")
    print("The patch file itself: chained-flow tree-patch --show")
    return 0
