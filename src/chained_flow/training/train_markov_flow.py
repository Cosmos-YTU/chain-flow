"""Training for the Markov-head hidden-space flow drafter (V6). Additive; reuses Trainer,
...
def markov_config_from_args(a: MarkovModelArguments) -> MarkovFlowConfig:
    return MarkovFlowConfig(
...
class MarkovTrainingModule(nn.Module):
    def __init__(self, frozen_lm: FrozenLMWrapper, dcfg: MarkovFlowConfig, loss_cfg: FlowLossArguments):
        super().__init__()
        self.drafter = MarkovFlowDrafter(frozen_lm, dcfg)
...
def train_markov_with_trainer(model_args, data_args, loss_args, training_args) -> dict[str, Any]:
    ...
    model = MarkovTrainingModule(frozen_lm, markov_config_from_args(model_args), loss_args)
    ...
    with (out / "chained_flow_markov_config.json").open("w") as f:
...
def load_markov_module(flow_dir, *, frozen_lm, device):
    flow_dir = Path(flow_dir)
    cfgp = flow_dir / "chained_flow_markov_config.json"
    if not cfgp.exists():
        cfgp = flow_dir.parent / "chained_flow_markov_config.json"
    config = json.load(open(cfgp))
    model_args = MarkovModelArguments(**config["model_args"])
    loss_args = FlowLossArguments(**config["loss_args"])
    module = MarkovTrainingModule(frozen_lm, markov_config_from_args(model_args), loss_args)
...
__all__ = ["MarkovModelArguments", "MarkovTrainingModule", "markov_config_from_args",
           "train_markov_with_trainer", "load_markov_module"]
