...
def train_refine_with_trainer(
    ...
    with (output_dir / "chained_flow_refine_config.json").open("w", encoding="utf-8") as f:
        json.dump(
...
def load_refine_module(flow_dir: str | Path, *, frozen_lm: FrozenLMWrapper, device) -> tuple[RefineTrainingModule, dict[str, Any]]:
    ...
    config_path = flow_dir / "chained_flow_refine_config.json"
    if not config_path.exists():
        config_path = flow_dir.parent / "chained_flow_refine_config.json"
