...
def train_twopass_with_trainer(model_args, data_args, loss_args, training_args) -> dict[str, Any]:
    ...
    with (out / "chained_flow_twopass_config.json").open("w") as f:
...
def load_twopass_module(flow_dir, *, frozen_lm, device):
    ...
    cfgp = flow_dir / "chained_flow_twopass_config.json"
    if not cfgp.exists():
        cfgp = flow_dir.parent / "chained_flow_twopass_config.json"
