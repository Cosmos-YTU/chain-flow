...
class FrozenLMWrapper:
    ...
    def from_pretrained(
        ...
        **kwargs: Any,
        ...
        # Vision-language backbones (e.g. Qwen3.6-27B) expose `language_model_only`; load
        # ONLY the text decoder so AutoModelForCausalLM yields the plain CausalLM (no vision encoder).
        if "config" not in model_kwargs and "language_model_only" not in model_kwargs:
            try:
                _cfg = AutoConfig.from_pretrained(model_id, local_files_only=local_files_only, trust_remote_code=trust_remote_code)
                if getattr(_cfg, "language_model_only", None) is not None:
                    _cfg.language_model_only = True
                    model_kwargs["config"] = _cfg
            except Exception:
                pass
