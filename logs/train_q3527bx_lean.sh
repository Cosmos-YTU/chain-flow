#!/bin/bash
# Qwen3.5-4B drafter, EXPANDED mix (tag 4bx) — LEAN: reuse existing stage1-4b technical teacher
...
st "START Qwen3.5-4B expanded-data pipeline (LEAN: reuse technical, collect 5 new) on GPUs 5+6"
...
tech=[f"teacher_states/stage1-q3527b-{x}" for x in ["gsm8k","nemotron-math","nemotron-stem","alpaca-code","dolly-chat"]]
new =[f"teacher_states/stage1-q3527bx-{x}" for x in ["ultrachat","no_robots","writing","summarization","translation"]]
parts=[]
for p in tech+new:
    d=load_from_disk(p); parts.append(d); print(f"  {p}: {len(d)} rows")
ds=concatenate_datasets(parts).shuffle(seed=0)
ds.save_to_disk("teacher_states/stage1-q3527bx-mix"); print("mix rows", len(ds))
PYEOF
...
$PY scripts/preprocess_flow_dataset.py --dataset-path teacher_states/stage1-$TAG-mix \
    --output-dir data/flow_cache/stage1_${TAG}_mix_k4 --draft-length 4 --hidden-dtype float16 --overwrite \
    2>&1 | grep -E "saved=|tokens="
[ -f data/flow_cache/stage1_${TAG}_mix_k4/hidden.pt ] || { echo FATAL drafter-cache; exit 1; }
...
CUDA_VISIBLE_DEVICES=5,6 $PY -m torch.distributed.run --nproc_per_node=2 --master_port=29521 \
