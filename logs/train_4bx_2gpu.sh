#!/bin/bash
# Qwen3.5-4B drafter on the EXPANDED, diversity-weighted mix (tag 4bx, 10 sources) — 2 GPUs (5+6).
#   parallel teacher collect (GPU5 || GPU6) -> combine 10 sources -> drafter+VAE caches
#   -> train VAE (1 GPU) -> train joint-VAE drafter (2-GPU DDP; effective batch 6144)
set -o pipefail
TAG=4bx
...
st "START Qwen3.5-4B expanded-data pipeline on GPUs 5+6"

# ---- 1. parallel teacher collection (config filenames); balanced 5/5 split ----
collect_one(){ # $1=gpu  $2..=config-names
  local gpu=$1; shift
  for s in "$@"; do
...
st "1/6 parallel collect (GPU5: gsm8k,nemotron_stem,alpaca_code,ultrachat,opus_translation | GPU6: nemotron_math,dolly,no_robots,writingprompts,cnn_dailymail)"
collect_one 5 gsm8k nemotron_stem alpaca_code ultrachat opus_translation > logs/collect_${TAG}_g5.log 2>&1 &
P5=$!
collect_one 6 nemotron_math dolly no_robots writingprompts cnn_dailymail > logs/collect_${TAG}_g6.log 2>&1 &
P6=$!
wait $P5; echo "g5 rc=$?"
wait $P6; echo "g6 rc=$?"
# guard: all 10 source dirs must exist (source-field names)
SRCS="gsm8k nemotron_math nemotron_stem alpaca_code dolly_chat ultrachat no_robots writing summarization translation"
for x in $SRCS; do
  [ -d "teacher_states/stage1-$TAG-$x" ] || { echo "FATAL missing source stage1-$TAG-$x"; exit 1; }
done
st "collection done — all 10 sources present"

export CUDA_VISIBLE_DEVICES=5

st "2/6 combine 10 sources -> mix"
$PY - <<PYEOF
from datasets import load_from_disk, concatenate_datasets
s="$SRCS".split()
ds=concatenate_datasets([load_from_disk(f'teacher_states/stage1-$TAG-{x}') for x in s]).shuffle(seed=0)
ds.save_to_disk('teacher_states/stage1-$TAG-mix'); print('mix rows', len(ds))
PYEOF
