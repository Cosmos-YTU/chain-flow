...
env -u CF_SHORTLIST -u CF_K -u CF_ASYNC_SCHED -u CF_CUDAGRAPH -u CF_DRAFTER_DIR -u VLLM_SPEC_TREE \
    CUDA_VISIBLE_DEVICES=$GPU PYTHONPATH=/home/shadeform/chained-flow/src CF_DRAFTER_DIR=$R \
...
grep -E "\[chained-flow\] (drafter from HF hub|shortlist head)" $OUT/server.log | tee $OUT/provenance.txt
