...
class TreeGDNLayout:
    ...
    __slots__ = ("chain", "depth", "seq", "loc", "maxd", "num_rows", "levels")
...
def layer_meta(
    ...
    m.out_slot = state_indices[lay.seq, col].to(torch.int32)
    m.init_slot = state_indices[lay.seq, ic_row].to(torch.int32)
    ...
    parent_row = lay.chain[:, 1].to(torch.int64)
    m.parent_slot = torch.where(
        lay.depth == 0, m.init_slot, m.out_slot[parent_row]
    ).to(torch.int32)
    m.levels = lay.levels
    return m
