...
def _tree_attention_fixup(
    ...
    scores = scores.masked_fill(~ts.anc_mask.unsqueeze(1), float("-inf"))
    ...
    flat = valid_pad.view(-1)
    suffix_out = suffix_out.reshape(S * nmax, H, D)[flat].to(output.dtype)
    suffix_lse = (
        suffix_lse.permute(0, 2, 1).reshape(S * nmax, H)[flat].transpose(0, 1)
    ).contiguous()
