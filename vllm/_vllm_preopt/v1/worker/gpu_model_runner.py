...
class GPUModelRunner(
    ...
    def _tree_postprocess(self, slot_mappings) -> None:
        ...
        if valid.any():
            ...
            dst_rows = idx_pad
            sel = valid.view(-1)
            src_rows = src_rows.reshape(-1)[sel]
            dst_rows = dst_rows.reshape(-1)[sel]
    ...
    def _prepare_inputs(
        ...
        if not use_spec_decode:
            ...
            if tree_branching:
                ...
                if _rows:
                    _ri = torch.tensor(_rows, dtype=torch.long, device=self.device)
                    _rv = torch.tensor(_newpos, dtype=torch.int64, device=self.device)
