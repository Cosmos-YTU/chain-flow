...
llm = LLM(model=MODEL, gpu_memory_utilization=0.88, max_model_len=2048, dtype="float16", enforce_eager=False, max_num_seqs=256)
...
def run(worker):
    ...
    # ---- LIGHT draft: shortlist head (knob1) + shorter depth (knob2); verify still lossless ----
    order_ = dcfg.path_order; Kd_ = dcfg.draft_length
    _SL = torch.load(os.environ["SHORTLIST"]).to(dev)
    _sub_lmw = lmw[_SL].contiguous().to(embed.dtype)
    _w2SL = drafter.markov.w2.weight[_SL].contiguous()
    _MAXD = int(os.environ.get("MAX_DEPTH", "5"))
    print(f"[LIGHT] shortlist K={_SL.numel()} max_depth={_MAXD}", flush=True)
    def _lmh(h): return h.to(_sub_lmw.dtype) @ _sub_lmw.T
    def _mbias(prev): return drafter.markov.w1(prev) @ _w2SL.T
    def draft_core(context, top_b=8, max_nodes=8, max_depth=None):
        max_depth = _MAXD if max_depth is None else max_depth
        pred_hidden = drafter.predict_hidden(context)[0]
        base0 = _lmh(pred_hidden[:1])[0]
        b0 = min(top_b, base0.shape[-1])
        vals, lidx = torch.log_softmax(base0.float(), dim=-1).topk(b0)
        idx = _SL[lidx]
        ...
        for d in range(1, min(Kd_, max_depth + 1)):
            ...
            logits = _lmh(pred_hidden[d].unsqueeze(0) + res) + _mbias(lastp[:, 0])
            cvals, clid = torch.log_softmax(logits.float(), dim=-1).topk(top_b, dim=-1)
            cand_cum = (cum[frontier].unsqueeze(1) + cvals).reshape(-1)
            keep = min(max_nodes, cand_cum.shape[0]); topv, topi = cand_cum.topk(keep)
            src_row = topi // top_b
            new_tokens = _SL[clid.reshape(-1)[topi]]; new_parents = frontier[src_row]
