...
class GPUModelRunner(
    ...
    def __init__(
        ...
        if (tree_state.tree_enabled() and self.num_spec_tokens > 0
    ...
    def _prepare_inputs(
        ...
        if not use_spec_decode:
            ...
            if _use_tree and num_draft_tokens.sum() > 0:
                _pp: list[np.ndarray] = []
                _dd: list[np.ndarray] = []
            ...
            if tree_branching:
                ...
                for r in range(num_reqs):
                    ...
                    base = int(self.input_batch.num_computed_tokens_cpu[r])
                    for i in range(n):
                        if int(d[i]) != i:
                            _rows.append(start + 1 + i)
                            _newpos.append(base + 1 + int(d[i]))
                if _rows:
                    ...
                    _rv = self._cf_h2d("tree_pos_vals", _newpos)
                    self.positions[:total_num_scheduled_tokens].index_copy_(0, _ri, _rv)
            ...
            if tree_parents_np is not None:
